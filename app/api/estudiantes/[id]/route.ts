...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    await db.execute({
      ...
      args: [nombre, apellido, email, edad ?? null, grado ?? null, fechaIngreso, activo, id],
    })
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  try {
    const id = Number(params.id)
    // Soft delete using activo flag
    await db.execute({ sql: "UPDATE estudiantes SET activo = 0 WHERE id = ?", args: [id] })
